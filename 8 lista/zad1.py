    ...
    def add_appointment(self, widget):
        with self.database.con:
            ...
            if self.checkboxType.get_active():
                ...
                type = 0
            cur.execute('INSERT INTO appointments (`name`, `content`, `type`, `start_date`, `end_date`) VALUES (?, ?, ?, ?, ?)', [unicode(self.entryName.get_text()), unicode(self.text.get_text(self.text.get_start_iter(), self.text.get_end_iter())), type, datetime.datetime.strptime(self.entryStartDate.get_text(), "%Y-%m-%d %H:%M"), datetime.datetime.strptime(self.entryEndDate.get_text(), "%Y-%m-%d %H:%M")])
        ...
        with self.database.con:
            ...
            for row in rows:
                ...
                if row[3] == 1:
                    ...
                    time = datetime.datetime.strptime(row[5][:-3], "%Y-%m-%d %H:%M")
        ...
        if result == 1:
            with self.database.con:
                ...
                if checkboxType.get_active():
                    ...
                    type = 0
                cur.execute('UPDATE appointments SET name=?, content=?, type=?, start_date=?, end_date=? WHERE id=?', [unicode(entryName.get_text()), unicode(text.get_text(text.get_start_iter(), text.get_end_iter())), type, datetime.datetime.strptime(entryStartDate.get_text(), "%Y-%m-%d %H:%M"), datetime.datetime.strptime(entryEndDate.get_text(), "%Y-%m-%d %H:%M"), id])
